...
class DiffusionOptimalTransport(Scene):
    ...
    def animate_diffusion(self):
        ...
        for p0, p1 in zip(self.alpha0, self.alpha1):
            particle = p0.copy()
            particle.add_updater(lambda m, p0=p0, p1=p1: m.move_to(
    ...
    def show_velocity_field(self):
        ...
        for p0, p1 in zip(self.alpha0, self.alpha1):
            ...
            arrow = Arrow(
                start=start_pos,
                end=start_pos + direction * 0.5,
                buff=0,
                color=WHITE,
                tip_length=0.1  # fixed for Manim 0.19.0
            )
    ...
    def show_continuity_equation(self):
        ...
        def deform_grid(mob, alpha):
            # iterate over submobjects instead of get_lines()
